...
def generate_answer(dialogue, question, memory=None):
    ...
    return gpt_4_mini_model.query(prompt, reset=True)

def process_qa(user_id, dialogue, question_wm, question_wom, gold_answer, memory, output_list, filename):
    """Processes QA pairs and writes results into a JSON file."""
    answer_wm = generate_answer(dialogue, question_wm, memory)
    answer_wom = generate_answer(dialogue, question_wom)
...
if __name__ == '__main__':
    ...
    for j in range(1,n+1):
        ...
        for i in range(1, k+1):
            ...
            memory = memory_update(memory,stage_dialogue,gpt_4_mini_model)
        # Processing Complex QA
        process_qa(Id, stage_dialogue, complex_question, complex_question, complex_gold_answer, memory, data_complex, "complex_QA.json")

        # Processing Simple QA
        process_qa(Id, stage_dialogue, simple_question, simple_question, simple_gold_answer, memory, data_simple, "simple_QA.json")
